...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
data_aper_withmeanzero=data_aper_3;
...
subplot(1, 3, 1)
bar(mean(mean_sorted_data_aper_withmeanzero(:, 9:33), 2));
...
% sort periodic data based on mean values from highest to lowest
sorted_data_per = data_per_3(sorted_indices_aper, :);
...
figure
bar(mean(mean_sorted_data_per(:, 9:33), 2),'r');
title('mean values of all periodic data in the order of Aperiodic high to low');
...
AperInsig_perSig= intersect(zero_indices_aper, non_zero_indices_per);
% find cells sig in periodic ,but not sig in aper
APerSig_perInSig = intersect(non_zero_indices_aper,zero_indices_per);
% number ofd cells in each caseA
numel(AperInsig_perSig)
numel(APerSig_perInSig)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
mean_diff_data_per=zeros(size(data_per_3,1),size(data_per_3{1,1},2));
diff_data_per=data_per_3;
...
title('periodic with zero mean values for insignificant cells');
